...
Функция ПрочитатьДанные(Data)
	...
	Если Данные.Свойство("callstatus") = Неопределено Тогда
...
Функция СоздатьДокументТелефонныйЗвонок(Данные)
	...
	ТелефонныйЗвонок.Тема = "Мобилон, " + ?(Данные.direction = "external", "исходящий вызов на: ", "входящий вызов с:") + Данные.ВнешнийНомер;
	ТелефонныйЗвонок.Комментарий = ТелефонныйЗвонок.Тема;
...
Процедура Event(Data) Экспорт
	...
	Если Данные.state = "HANGUP" И НЕ ЗначениеЗаполнено(Данные.Документ) Тогда
